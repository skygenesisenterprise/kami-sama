...
function artworkTypeIcon(type: ArtworkType) {
  ...
  if (type === 'banner') return <Film className="size-4" />
  if (type === 'thumb') return <FileText className="size-4" />
...
function OverviewTab({ data }: { data: TvdbProviderData }) {
  ...
  const statCards = [
    { label: 'Total Series', value: stats.totalSeries.toLocaleString(), sub: 'series indexed', icon: Tv },
    { label: 'Total Episodes', value: stats.totalEpisodes.toLocaleString(), sub: 'episodes tracked', icon: Film },
    { label: 'Total Artwork', value: stats.totalArtwork.toLocaleString(), sub: 'artwork assets', icon: Image },
    { label: 'Cached', value: stats.cachedEntries.toLocaleString(), sub: `of ${stats.totalSeries.toLocaleString()} series`, icon: Database },
        ...
        <CardHeader className="flex flex-row items-center gap-4 space-y-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              {provider.version} &middot; {provider.apiUrl} &middot; {provider.seriesCount.toLocaleString()} series &middot; Avg {stats.averageResponseMs}ms
            </p>
          </div>
          <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {cat.count.toLocaleString()} items &middot; {cat.syncedCount.toLocaleString()} synced
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Auto-sync artwork for new series</p>
              </div>
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <p className="text-sm font-medium">Include Fanart</p>
                <p className="text-xs text-muted-foreground">Sync fanart from Fanart.tv integration</p>
              </div>
              <Switch defaultChecked />
            </div>
          </div>
...
function JobsTab({ jobs }: { jobs: SyncJob[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-2">
        {['all', 'series-sync', 'episode-fetch', 'artwork-fetch', 'batch-import', 'cleanup'].map((t) => (
          <Button
            key={t}
            size="sm"
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex size-8 items-center justify-center rounded-md bg-muted">
                    {job.type === 'series-sync' && <Tv className="size-4" />}
                    {job.type === 'episode-fetch' && <Film className="size-4" />}
                    {job.type === 'artwork-fetch' && <Image className="size-4" />}
                    {job.type === 'batch-import' && <Download className="size-4" />}
  ...
  const filtered = search
    ? mappings.filter(
        (m) =>
          ...
          m.tvdbId.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <Input
          ...
          className="max-w-sm"
        />
        <Button size="sm" variant="outline" onClick={() => toast.info('Scan for new series...')}>
            ...
            <TableBody>
              {filtered.map((m) => (
                <TableRow key={m.id}>
                  ...
                  <TableCell>
                    <p className="font-medium text-xs font-mono">{m.tvdbId}</p>
                  </TableCell>
                  <TableCell>
                    <Badge variant={m.seriesStatus === 'continuing' ? 'default' : 'secondary'} className="text-[10px] capitalize">
                      {m.seriesStatus}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-sm">{m.seasons}</TableCell>
                  <TableCell className="text-sm">{m.episodes}</TableCell>
                  <TableCell>
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field>
              ...
              <Input type="password" value={form.apiKey} onChange={(e) => update('apiKey', e.target.value)} placeholder="Your TVDB API key" />
            </Field>
            <Field>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field>
              ...
              <Input type="number" value={form.rateLimitPerSecond} onChange={(e) => update('rateLimitPerSecond', parseInt(e.target.value) || 20)} />
            </Field>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Cache series data locally to reduce API calls</p>
              </div>
              <Switch checked={form.cacheEnabled} onCheckedChange={(v) => update('cacheEnabled', v)} />
            </div>
            <Field>
              <FieldLabel>Cache TTL (hours)</FieldLabel>
              <Input type="number" value={form.cacheTtlHours} onChange={(e) => update('cacheTtlHours', parseInt(e.target.value) || 72)} disabled={!form.cacheEnabled} />
            </Field>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field>
              <FieldLabel>Max Cache Size (MB)</FieldLabel>
              <Input type="number" value={form.maxCacheSizeMb} onChange={(e) => update('maxCacheSizeMb', parseInt(e.target.value) || 8192)} disabled={!form.cacheEnabled} />
            </Field>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Automatically sync series data on schedule</p>
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <Field>
              <FieldLabel>Artwork Language</FieldLabel>
              <Input value={form.artworkLanguage} onChange={(e) => update('artworkLanguage', e.target.value)} placeholder="en" />
            </Field>
            <Field>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <p className="text-sm font-medium">Include Artwork</p>
                <p className="text-xs text-muted-foreground">Sync posters, banners, and fanart</p>
              </div>
              <Switch checked={form.includeArtwork} onCheckedChange={(v) => update('includeArtwork', v)} />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <p className="text-sm font-medium">Fetch Episodes</p>
                <p className="text-xs text-muted-foreground">Sync episode-level metadata</p>
              </div>
              <Switch checked={form.fetchEpisodes} onCheckedChange={(v) => update('fetchEpisodes', v)} />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <p className="text-sm font-medium">Fetch Actors</p>
                <p className="text-xs text-muted-foreground">Sync cast and crew data</p>
              </div>
              <Switch checked={form.fetchActors} onCheckedChange={(v) => update('fetchActors', v)} />
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="flex items-center justify-between rounded-lg border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Receive notifications when new data is available</p>
            </div>
...
export default function TvdbProviderPage() {
  const data = tvdbProviderData

  return (
    <main className="flex flex-col gap-6">
      <PageHeader
        title="TheTVDB"
        description="Configure and monitor your TVDB integration. Fetch series, episode data, season artwork, and metadata for your TV library."
      >
