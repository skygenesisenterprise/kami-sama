...
function mediaTypeIcon(type: MediaType) {
  ...
  if (type === 'tv') return <Tv className="size-4" />
...
function OverviewTab({ data }: { data: TmdbProviderData }) {
  ...
  const statCards = [
    { label: 'Total Media', value: stats.totalMedia.toLocaleString(), sub: 'items indexed', icon: Film },
    { label: 'Movies', value: stats.movieCount.toLocaleString(), sub: 'films tracked', icon: Film },
    { label: 'TV Shows', value: stats.tvCount.toLocaleString(), sub: 'series tracked', icon: Tv },
    { label: 'Anime', value: stats.animeCount.toLocaleString(), sub: 'anime tracked', icon: Zap },
        ...
        <CardHeader className="flex flex-row items-center gap-4 space-y-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              {provider.version} &middot; {provider.apiUrl} &middot; {provider.mediaCount.toLocaleString()} media &middot; Avg {stats.averageResponseMs}ms
            </p>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-xs text-muted-foreground">{cachePercent}% utilized &middot; {stats.cachedMetadata.toLocaleString()} metadata cached</p>
          </CardContent>
        </Card>
      </div>
    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {cat.count.toLocaleString()} items &middot; {cat.language.toUpperCase()}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Automatically sync metadata for new media</p>
              </div>
              <Switch defaultChecked />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <p className="text-sm font-medium">Include Anime</p>
                <p className="text-xs text-muted-foreground">Sync anime-specific metadata fields</p>
              </div>
              <Switch defaultChecked />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <p className="text-sm font-medium">Include Adult</p>
                <p className="text-xs text-muted-foreground">Fetch adult-rated content metadata</p>
              </div>
              <Switch />
            </div>
          </div>
...
function JobsTab({ jobs }: { jobs: SyncJob[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-2">
        {['all', 'metadata-sync', 'cast-fetch', 'trailer-fetch', 'batch-import', 'cleanup'].map((t) => (
          <Button
            key={t}
            size="sm"
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex size-8 items-center justify-center rounded-md bg-muted">
                    {job.type === 'metadata-sync' && <RefreshCw className="size-4" />}
                    {job.type === 'cast-fetch' && <Users className="size-4" />}
                    {job.type === 'trailer-fetch' && <Film className="size-4" />}
  ...
  const filtered = search
    ? mappings.filter(
        (m) =>
          ...
          m.tmdbId.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <Input
          ...
          className="max-w-sm"
        />
        <Button size="sm" variant="outline" onClick={() => toast.info('Scan for new media...')}>
            ...
            <TableBody>
              {filtered.map((m) => (
                <TableRow key={m.id}>
                  ...
                  <TableCell>
                    <div>
                      <p className="font-medium text-xs font-mono">{m.tmdbId}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      ...
                      {m.externalIds.mal && <Badge variant="outline" className="text-[10px]">MAL: {m.externalIds.mal}</Badge>}
                    </div>
                  </TableCell>
                  <TableCell>{matchScoreBadge(m.matchScore)}</TableCell>
                  <TableCell>
                    <p className="max-w-48 truncate text-xs text-muted-foreground">{m.overview}</p>
                  </TableCell>
...
function SettingsTab({ settings }: { settings: ProviderSettings }) {
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field>
              ...
              <Input type="password" value={form.apiKey} onChange={(e) => update('apiKey', e.target.value)} placeholder="Your TMDB API key" />
            </Field>
            <Field>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field>
              ...
              <Input type="number" value={form.rateLimitPerSecond} onChange={(e) => update('rateLimitPerSecond', parseInt(e.target.value) || 40)} />
            </Field>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Cache metadata locally to reduce API calls</p>
              </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Metadata Fetching</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Automatically sync metadata on schedule</p>
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div>
                <p className="text-sm font-medium">Include Anime</p>
                <p className="text-xs text-muted-foreground">Sync anime-specific metadata</p>
              </div>
              <Switch checked={form.includeAnime} onCheckedChange={(v) => update('includeAnime', v)} />
            </div>
          </div>
          <div className="flex items-center justify-between rounded-lg border p-3">
            <div>
              <p className="text-sm font-medium">Include Adult Content</p>
              <p className="text-xs text-muted-foreground">Fetch metadata for adult-rated content</p>
            </div>
            <Switch checked={form.includeAdult} onCheckedChange={(v) => update('includeAdult', v)} />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="flex items-center justify-between rounded-lg border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Receive notifications when new metadata is available</p>
            </div>
...
const TABS = [
  ...
  { value: 'categories', label: 'Categories', icon: Film },
...
export default function TmdbProviderPage() {
  const data = tmdbProviderData

  return (
    <main className="flex flex-col gap-6">
      <PageHeader
        title="The Movie Database"
        description="Configure and monitor your TMDB integration. Fetch metadata, cast, trailers and ratings for your media library."
      >
