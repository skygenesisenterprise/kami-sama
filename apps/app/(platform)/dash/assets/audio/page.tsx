...
type SortKey = 'title' | 'format' | 'size' | 'duration' | 'usage' | 'updated' | 'imported'
...
const MOCK_IMPORT_PREVIEWS: ImportPreview[] = [
  { id: 'imp-1', source: 'spotify', title: 'Bling-Bang-Bang-Born - Creepy Nuts', url: '/audio/mashle-op1-import.mp3', thumbnailUrl: '/audio/thumbs/mashle-op1-import.jpg', format: 'mp3', sizeBytes: 8_500_000, language: 'ja', alreadyExists: false, selected: true, durationSeconds: 185, artist: 'Creepy Nuts' },
  { id: 'imp-2', source: 'spotify', title: 'SPECIALZ - King Gnu', url: '/audio/jjk-s2-op1-import.flac', thumbnailUrl: '/audio/thumbs/jjk-s2-op1-import.jpg', format: 'flac', sizeBytes: 36_000_000, language: 'ja', alreadyExists: true, selected: false, durationSeconds: 240, artist: 'King Gnu' },
  { id: 'imp-3', source: 'anilist', title: 'Ao no Sumika - Tatsuya Kitani', url: '/audio/jjk-op1-import.mp3', thumbnailUrl: '/audio/thumbs/jjk-op1-import.jpg', format: 'mp3', sizeBytes: 8_200_000, language: 'ja', alreadyExists: false, selected: true, durationSeconds: 225, artist: 'Tatsuya Kitani' },
  { id: 'imp-4', source: 'spotify', title: 'Idol - YOASOBI', url: '/audio/oshi-no-ko-op1-import.flac', thumbnailUrl: '/audio/thumbs/oshi-no-ko-op1-import.jpg', format: 'flac', sizeBytes: 38_000_000, language: 'ja', alreadyExists: false, selected: true, durationSeconds: 265, artist: 'YOASObi' },
  { id: 'imp-5', source: 'spotify', title: 'Mob Psycho 100 - Mob Choir', url: '/audio/mob-psycho-op3-import.mp3', thumbnailUrl: '/audio/thumbs/mob-psycho-op3-import.jpg', format: 'mp3', sizeBytes: 7_800_000, language: 'ja', alreadyExists: true, selected: false, durationSeconds: 195, artist: 'Mob Choir' },
]
...
export default function AudioPage() {
  ...
  const [inspecting, setInspecting] = React.useState<AudioTrack | null>(null)
  const [importOpen, setImportOpen] = React.useState(false)
  const [settingsOpen, setSettingsOpen] = React.useState(false)
  const [settings, setSettings] = React.useState<AudioSettings>(MOCK_AUDIO_SETTINGS)

  const stats = React.useMemo(() => {
    const total = MOCK_AUDIO.length
    const storage = MOCK_AUDIO.reduce((s, a) => s + a.sizeBytes, 0)
    const unused = MOCK_AUDIO.filter((a) => a.status === 'unused').length
    const missing = MOCK_AUDIO.filter((a) => a.status === 'missing').length
    return { total, storage, unused, missing }
  }, [])

  const filtered = React.useMemo(() => {
    let items = MOCK_AUDIO.filter((item) => {
      const matchesQuery = item.title.toLowerCase().includes(query.toLowerCase()) || (item.artist && item.artist.toLowerCase().includes(query.toLowerCase()))
  ...
  return (
    <main className="flex flex-col gap-6">
      <PageHeader
        title="Audio"
        description="Manage and organize all audio tracks across the platform."
      >
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard label="Total Tracks" value={stats.total} icon={<Music className="size-4 text-muted-foreground" />} />
        <StatCard label="Storage Used" value={formatBytes(stats.storage)} icon={<Headphones className="size-4 text-muted-foreground" />} />
        ...
        <Select value={mediaTypeFilter} onValueChange={setMediaTypeFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {AUDIO_MEDIA_TYPES.map((t) => (
                <SelectItem key={t} value={t}>{AUDIO_MEDIA_TYPE_LABEL[t]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
        ...
        <Select value={sourceFilter} onValueChange={setSourceFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {AUDIO_SOURCES.map((s) => (
                <SelectItem key={s} value={s}>{AUDIO_SOURCE_LABEL[s]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {AUDIO_LANGUAGES.map((l) => (
                <SelectItem key={l} value={l}>{AUDIO_LANGUAGE_LABEL[l]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {AUDIO_STATUSES.map((s) => (
                <SelectItem key={s} value={s}>{AUDIO_STATUS_LABEL[s]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
      ...
      {filtered.length === 0 ? (
        <Empty className="border">
          <EmptyHeader>
            ...
            <EmptyTitle>No audio tracks found</EmptyTitle>
            <EmptyDescription>No audio tracks match the current filters.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : view === 'grid' ? (
        <AudioGrid items={filtered} onInspect={setInspecting} />
      ) : view === 'table' ? (
        <AudioTable items={filtered} onInspect={setInspecting} />
      ) : (
        <AudioGallery items={filtered} onInspect={setInspecting} />
      )}

      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>Showing {filtered.length} of {MOCK_AUDIO.length} tracks</span>
      ...
      <AudioDetailSheet item={inspecting} onClose={() => setInspecting(null)} />
...
function AudioPlaceholder({ title, className = '' }: { title: string; className?: string }) {
  return (
...
function AudioGrid({ items, onInspect }: { items: AudioTrack[]; onInspect: (item: AudioTrack) => void }) {
  return (
      ...
      {items.map((item) => (
        ...
        >
          <div className="relative aspect-square overflow-hidden rounded-md bg-muted">
            {item.status === 'missing' ? (
              <AudioPlaceholder title="Missing" className="size-full" />
            ) : (
              <AudioPlaceholder title={item.title} className="size-full" />
            )}
            ...
            <div className="absolute left-2 top-2 flex gap-1">
              <Badge variant={item.status === 'active' ? 'default' : item.status === 'missing' ? 'destructive' : 'secondary'} className="text-[10px]">
                {AUDIO_STATUS_LABEL[item.status]}
              </Badge>
              {item.isDuplicate && (
          ...
          <div className="flex flex-col gap-1">
            ...
            {item.artist && (
              <span className="line-clamp-1 text-xs text-muted-foreground">{item.artist}</span>
            )}
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>{AUDIO_MEDIA_TYPE_LABEL[item.mediaType]}</span>
              <span>·</span>
...
function AudioTable({ items, onInspect }: { items: AudioTrack[]; onInspect: (item: AudioTrack) => void }) {
  return (
            ...
            <TableRow key={item.id}>
              <TableCell>
                <button type="button" className="size-10 shrink-0 overflow-hidden rounded bg-muted" onClick={() => onInspect(item)}>
                  <AudioPlaceholder title="" className="size-full" />
                </button>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="hidden md:table-cell">
                <span className="text-xs">{item.artist || 'N/A'}</span>
              </TableCell>
              <TableCell className="hidden md:table-cell">
                <Badge variant="outline" className="text-xs">{AUDIO_MEDIA_TYPE_LABEL[item.mediaType]}</Badge>
              </TableCell>
              ...
              <TableCell className="hidden xl:table-cell">
                <span className="text-xs">{AUDIO_SOURCE_LABEL[item.source]}</span>
              </TableCell>
              <TableCell className="hidden xl:table-cell">
                <span className="font-mono text-xs">{item.usageCount}</span>
              </TableCell>
              <TableCell className="hidden xl:table-cell">
                <StatusBadge tone={AUDIO_STATUS_TONE[item.status]}>{AUDIO_STATUS_LABEL[item.status]}</StatusBadge>
              </TableCell>
...
function AudioGallery({ items, onInspect }: { items: AudioTrack[]; onInspect: (item: AudioTrack) => void }) {
      ...
      {selected && (
        <Card className="p-4">
          <div className="flex items-center gap-4">
            <div className="aspect-square w-32 shrink-0 overflow-hidden rounded-lg bg-muted">
              <AudioPlaceholder title="Preview" className="size-full" />
            </div>
            <div className="flex-1">
              <h3 className="font-medium">{MOCK_AUDIO.find((a) => a.id === selected)?.title}</h3>
              <p className="text-sm text-muted-foreground">
                {MOCK_AUDIO.find((a) => a.id === selected)?.artist} · {formatDuration(MOCK_AUDIO.find((a) => a.id === selected)?.durationSeconds || 0)} · {formatBytes(MOCK_AUDIO.find((a) => a.id === selected)?.sizeBytes || 0)}
              </p>
              <div className="mt-2 flex gap-2">
                <Button size="sm" onClick={() => onInspect(MOCK_AUDIO.find((a) => a.id === selected)!)}>
        ...
        {items.map((item) => (
          ...
          >
            <div className="aspect-square bg-muted relative">
              {item.status === 'missing' ? (
                <AudioPlaceholder title="Missing" className="size-full rounded-t-lg" />
              ) : (
                <AudioPlaceholder title={item.title} className="size-full rounded-t-lg" />
              )}
            ...
            <div className="p-2">
              ...
              {item.artist && <span className="line-clamp-1 text-[10px] text-muted-foreground">{item.artist}</span>}
...
function AudioDetailSheet({ item, onClose }: { item: AudioTrack | null; onClose: () => void }) {
  return (
          ...
          <>
            <SheetHeader className="border-b">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>{formatBytes(item.sizeBytes)}</span>
                <span>·</span>
                <span>{AUDIO_SOURCE_LABEL[item.source]}</span>
              </div>
            </SheetHeader>
            <Tabs defaultValue="overview" className="flex-1 overflow-y-auto px-4 py-4">
              ...
              <TabsContent value="overview" className="pt-4">
                <FieldGroup>
                  <div className="aspect-square w-full max-w-xs mx-auto overflow-hidden rounded-lg bg-muted">
                    {item.status === 'missing' ? (
                      <AudioPlaceholder title="Missing" className="size-full" />
                    ) : (
                      <AudioPlaceholder title={item.title} className="size-full" />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <MiniStat label="Artist" value={item.artist || 'N/A'} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <MiniStat label="Language" value={AUDIO_LANGUAGE_LABEL[item.language]} />
                ...
                <FieldGroup>
                  ...
                  {item.usedBy.length === 0 ? (
                    <div className="rounded-lg border px-3 py-6 text-center text-sm text-muted-foreground">
                      This track is not used by any resource
                    </div>
                  ) : (
                        ...
                        <div key={v.id} className="flex items-center justify-between rounded-lg border px-3 py-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-muted-foreground">{AUDIO_LANGUAGE_LABEL[v.language]}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-4">
                            <span className="font-mono text-xs">{formatDuration(v.durationSeconds)}</span>
                            <span className="font-mono text-xs">{v.bitrate} kbps</span>
              ...
              <TabsContent value="metadata" className="pt-4">
                <FieldGroup>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <MiniStat label="Sample Rate" value={`${item.metadata.sampleRate} Hz`} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <MiniStat label="Channels" value={item.metadata.channels === 1 ? 'Mono' : 'Stereo'} />
                    <MiniStat label="Encoder" value={item.metadata.encoder || 'N/A'} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <MiniStat label="Genre" value={item.metadata.genre || 'N/A'} />
                    <MiniStat label="Year" value={item.metadata.year?.toString() || 'N/A'} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                      ...
                      <div key={entry.id} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          <div className={cn(
                            ...
                            entry.action === 'normalize' && 'bg-success',
            ...
            <SheetFooter className="flex-row border-t">
              <div className="flex flex-1 flex-wrap gap-2">
                ...
                <Button variant="outline" size="sm"><Volume2 data-icon="inline-start" /> Normalize</Button>
              </div>
...
function ImportDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const [source, setSource] = React.useState<ImportSource>('spotify')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Select a source and search for audio tracks to import.</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          <Field>
            ...
            {previews.map((item) => (
              ...
              >
                ...
                {item.alreadyExists ? (
                  ...
                  <Badge variant="secondary" className="text-[10px]">{item.format.toUpperCase()}</Badge>
                )}
              </div>
            ))}
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() => {
              toast.success(`${selectedCount} track(s) imported`)
              onOpenChange(false)
            }}
          >
            <Upload data-icon="inline-start" />
...
function SettingsDialog({ open, onOpenChange, settings, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; settings: AudioSettings; onSave: (settings: AudioSettings) => void }) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Configure how audio tracks are managed and processed.</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          <Field>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field>
              <FieldLabel>Normalize Audio</FieldLabel>
              <Select value={local.normalizeAudio ? 'yes' : 'no'} onValueChange={(v) => setLocal({ ...local, normalizeAudio: v === 'yes' })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="yes">Yes</SelectItem>
                  <SelectItem value="no">No</SelectItem>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field>
              <FieldLabel>Target Loudness (LUFS)</FieldLabel>
              <Input type="number" value={local.targetLoudness} onChange={(e) => setLocal({ ...local, targetLoudness: Number(e.target.value) })} />
            </Field>
            <Field>
              <FieldLabel>Compression Quality (kbps)</FieldLabel>
