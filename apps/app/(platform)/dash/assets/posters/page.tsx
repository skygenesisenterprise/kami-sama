...
import {
  ...
  POSTER_RESOLUTIONS,
  POSTER_RESOLUTION_LABEL,
  POSTER_RESOLUTION_TONE,
  POSTER_FORMATS,
  POSTER_STATUSES,
  POSTER_STATUS_LABEL,
  POSTER_STATUS_TONE,
...
const MOCK_IMPORT_PREVIEWS: ImportPreview[] = [
  { id: 'imp-1', source: 'tmdb', title: 'One Punch Man Season 3', url: '/posters/opm-s3-import.jpg', thumbnailUrl: '/posters/thumbs/opm-s3-import.jpg', resolution: '4k', format: 'jpg', width: 2160, height: 3240, sizeBytes: 4_500_000, language: 'en', alreadyExists: false, selected: true },
  { id: 'imp-2', source: 'tmdb', title: 'Chainsaw Man Part 2', url: '/posters/csm-p2-import.jpg', thumbnailUrl: '/posters/thumbs/csm-p2-import.jpg', resolution: '4k', format: 'jpg', width: 2160, height: 3240, sizeBytes: 3_800_000, language: 'en', alreadyExists: true, selected: false },
  { id: 'imp-3', source: 'anilist', title: 'Frieren Season 2', url: '/posters/frieren-s2-import.jpg', thumbnailUrl: '/posters/thumbs/frieren-s2-import.jpg', resolution: '1080p', format: 'png', width: 1080, height: 1620, sizeBytes: 2_100_000, language: 'ja', alreadyExists: false, selected: true },
  { id: 'imp-4', source: 'fanart', title: 'Spy x Family Movie 2', url: '/posters/spy-family-m2-import.jpg', thumbnailUrl: '/posters/thumbs/spy-family-m2-import.jpg', resolution: '4k', format: 'webp', width: 2160, height: 3240, sizeBytes: 2_500_000, language: 'en', alreadyExists: false, selected: true },
  { id: 'imp-5', source: 'tmdb', title: 'Solo Leveling Season 2', url: '/posters/solo-leveling-s2-import.jpg', thumbnailUrl: '/posters/thumbs/solo-leveling-s2-import.jpg', resolution: '4k', format: 'jpg', width: 2160, height: 3240, sizeBytes: 4_200_000, language: 'fr', alreadyExists: true, selected: false },
]
...
export default function PostersPage() {
  ...
  const [inspecting, setInspecting] = React.useState<Poster | null>(null)
  const [importOpen, setImportOpen] = React.useState(false)
  const [settingsOpen, setSettingsOpen] = React.useState(false)
  const [settings, setSettings] = React.useState<PosterSettings>(MOCK_POSTER_SETTINGS)

  const stats = React.useMemo(() => {
    const total = MOCK_POSTERS.length
    const storage = MOCK_POSTERS.reduce((s, p) => s + p.sizeBytes, 0)
    const unused = MOCK_POSTERS.filter((p) => p.status === 'unused').length
    const missing = MOCK_POSTERS.filter((p) => p.status === 'missing').length
    return { total, storage, unused, missing }
  }, [])

  const filtered = React.useMemo(() => {
    let items = MOCK_POSTERS.filter((item) => {
  ...
  return (
    <main className="flex flex-col gap-6">
      <PageHeader
        title="Posters"
        description="Manage and organize all poster assets across the platform."
      >
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          <span>{stats.total} posters</span>
          <span>·</span>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard label="Total Posters" value={stats.total} icon={<ImageIcon className="size-4 text-muted-foreground" />} />
        ...
        <Select value={mediaTypeFilter} onValueChange={setMediaTypeFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {POSTER_MEDIA_TYPES.map((t) => (
                <SelectItem key={t} value={t}>{POSTER_MEDIA_TYPE_LABEL[t]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
        ...
        <Select value={resolutionFilter} onValueChange={setResolutionFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {POSTER_RESOLUTIONS.map((r) => (
                <SelectItem key={r} value={r}>{POSTER_RESOLUTION_LABEL[r]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
        ...
        <Select value={sourceFilter} onValueChange={setSourceFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {POSTER_SOURCES.map((s) => (
                <SelectItem key={s} value={s}>{POSTER_SOURCE_LABEL[s]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
        ...
        <Select value={languageFilter} onValueChange={setLanguageFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {POSTER_LANGUAGES.map((l) => (
                <SelectItem key={l} value={l}>{POSTER_LANGUAGE_LABEL[l]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {POSTER_STATUSES.map((s) => (
                <SelectItem key={s} value={s}>{POSTER_STATUS_LABEL[s]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
      ...
      {filtered.length === 0 ? (
        <Empty className="border">
          <EmptyHeader>
            ...
            <EmptyTitle>No posters found</EmptyTitle>
            <EmptyDescription>No posters match the current filters.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : view === 'grid' ? (
        <PostersGrid items={filtered} onInspect={setInspecting} />
      ) : view === 'table' ? (
        <PostersTable items={filtered} onInspect={setInspecting} />
      ) : (
        <PostersGallery items={filtered} onInspect={setInspecting} />
      )}

      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>Showing {filtered.length} of {MOCK_POSTERS.length} posters</span>
      ...
      <PosterDetailSheet item={inspecting} onClose={() => setInspecting(null)} />
...
function PostersGrid({ items, onInspect }: { items: Poster[]; onInspect: (item: Poster) => void }) {
  return (
      ...
      {items.map((item) => (
        ...
        >
          <div className="relative aspect-2/3 overflow-hidden rounded-md bg-muted">
            {item.status === 'missing' ? (
              <PosterPlaceholder title="Missing" className="size-full" />
            ) : (
              <PosterPlaceholder title={item.title} className="size-full" />
            )}
            <div className="absolute left-2 top-2 flex gap-1">
              <Badge variant={item.status === 'active' ? 'default' : item.status === 'missing' ? 'destructive' : 'secondary'} className="text-[10px]">
                {POSTER_STATUS_LABEL[item.status]}
              </Badge>
                {item.isDuplicate && (
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>{POSTER_MEDIA_TYPE_LABEL[item.mediaType]}</span>
              <span>·</span>
              <span>{item.format.toUpperCase()}</span>
              <span>·</span>
              <span>{POSTER_SOURCE_LABEL[item.source]}</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
...
function PostersTable({ items, onInspect }: { items: Poster[]; onInspect: (item: Poster) => void }) {
  return (
            ...
            <TableRow key={item.id}>
              <TableCell>
                <button type="button" className="size-10 shrink-0 overflow-hidden rounded bg-muted" onClick={() => onInspect(item)}>
                  <PosterPlaceholder title="" className="size-full" />
                </button>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="hidden md:table-cell">
                <Badge variant="outline" className="text-xs">{POSTER_MEDIA_TYPE_LABEL[item.mediaType]}</Badge>
              </TableCell>
              <TableCell className="hidden md:table-cell">
                <StatusBadge tone={POSTER_RESOLUTION_TONE[item.resolution]}>{POSTER_RESOLUTION_LABEL[item.resolution]}</StatusBadge>
              </TableCell>
              ...
              <TableCell className="hidden xl:table-cell">
                <span className="text-xs">{POSTER_SOURCE_LABEL[item.source]}</span>
              </TableCell>
              <TableCell className="hidden xl:table-cell">
                <span className="font-mono text-xs">{item.usageCount}</span>
              </TableCell>
              <TableCell className="hidden xl:table-cell">
                <StatusBadge tone={POSTER_STATUS_TONE[item.status]}>{POSTER_STATUS_LABEL[item.status]}</StatusBadge>
              </TableCell>
...
function PostersGallery({ items, onInspect }: { items: Poster[]; onInspect: (item: Poster) => void }) {
      ...
      {selected && (
        <Card className="p-4">
          <div className="flex items-center gap-4">
            <div className="aspect-2/3 w-32 shrink-0 overflow-hidden rounded-lg bg-muted">
              <PosterPlaceholder title="Preview" className="size-full" />
            </div>
            <div className="flex-1">
              <h3 className="font-medium">{MOCK_POSTERS.find((p) => p.id === selected)?.title}</h3>
              <p className="text-sm text-muted-foreground">
                {MOCK_POSTERS.find((p) => p.id === selected)?.width}×{MOCK_POSTERS.find((p) => p.id === selected)?.height} · {formatBytes(MOCK_POSTERS.find((p) => p.id === selected)?.sizeBytes || 0)}
              </p>
              <div className="mt-2 flex gap-2">
                <Button size="sm" onClick={() => onInspect(MOCK_POSTERS.find((p) => p.id === selected)!)}>
        ...
        {items.map((item) => (
          ...
          >
            <div className="aspect-2/3 bg-muted">
              {item.status === 'missing' ? (
                <PosterPlaceholder title="Missing" className="size-full rounded-t-lg" />
              ) : (
                <PosterPlaceholder title={item.title} className="size-full rounded-t-lg" />
              )}
            </div>
            <div className="p-2">
...
function PosterDetailSheet({ item, onClose }: { item: Poster | null; onClose: () => void }) {
  return (
          ...
          <>
            ...
            <Tabs defaultValue="overview" className="flex-1 overflow-y-auto px-4 py-4">
              ...
              <TabsContent value="overview" className="pt-4">
                <FieldGroup>
                  <div className="aspect-2/3 w-full max-w-xs mx-auto overflow-hidden rounded-lg bg-muted">
                    {item.status === 'missing' ? (
                      <PosterPlaceholder title="Missing" className="size-full" />
                    ) : (
                      <PosterPlaceholder title={item.title} className="size-full" />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <MiniStat label="Language" value={POSTER_LANGUAGE_LABEL[item.language]} />
                ...
                <FieldGroup>
                  ...
                  {item.usedBy.length === 0 ? (
                    <div className="rounded-lg border px-3 py-6 text-center text-sm text-muted-foreground">
                      This poster is not used by any resource
                    </div>
                  ) : (
                        ...
                        <div key={v.id} className="flex items-center justify-between rounded-lg border px-3 py-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-muted-foreground">{POSTER_LANGUAGE_LABEL[v.language]}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-4">
              ...
              <TabsContent value="metadata" className="pt-4">
                <FieldGroup>
                  <div className="grid grid-cols-2 gap-4">
                    <MiniStat label="Resolution" value={POSTER_RESOLUTION_LABEL[item.resolution]} />
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Import Posters</DialogTitle>
          <DialogDescription>Select a source and search for posters to import.</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          <Field>
            ...
            {previews.map((item) => (
              ...
              >
                ...
                {item.alreadyExists ? (
                  ...
                  <Badge variant="secondary" className="text-[10px]">{POSTER_LANGUAGE_LABEL[item.language]}</Badge>
                )}
              </div>
            ))}
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() => {
              toast.success(`${selectedCount} poster(s) imported`)
              onOpenChange(false)
            }}
          >
            <Upload data-icon="inline-start" />
...
function SettingsDialog({ open, onOpenChange, settings, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; settings: PosterSettings; onSave: (settings: PosterSettings) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Poster Settings</DialogTitle>
          <DialogDescription>Configure how posters are managed and processed.</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          <Field>
