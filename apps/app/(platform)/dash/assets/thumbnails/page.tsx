...
import {
  MOCK_THUMBNAILS,
  MOCK_THUMBNAIL_SETTINGS,
  THUMBNAIL_MEDIA_TYPES,
  THUMBNAIL_MEDIA_TYPE_LABEL,
  THUMBNAIL_RESOLUTIONS,
  THUMBNAIL_RESOLUTION_LABEL,
  THUMBNAIL_RESOLUTION_TONE,
  THUMBNAIL_FORMATS,
  THUMBNAIL_STATUSES,
  THUMBNAIL_STATUS_LABEL,
  THUMBNAIL_STATUS_TONE,
  THUMBNAIL_SOURCES,
  THUMBNAIL_SOURCE_LABEL,
  THUMBNAIL_LANGUAGES,
  THUMBNAIL_LANGUAGE_LABEL,
  IMPORT_SOURCES,
  IMPORT_SOURCE_LABEL,
  QUALITY_LABEL,
  formatBytes,
...
const MOCK_IMPORT_PREVIEWS: ImportPreview[] = [
  { id: 'imp-1', source: 'tmdb', title: 'One Punch Man S3 E1', url: '/thumbnails/opm-s3-import.jpg', thumbnailUrl: '/thumbnails/thumbs/opm-s3-import.jpg', resolution: '4k', format: 'jpg', width: 1920, height: 1080, sizeBytes: 850_000, language: 'en', alreadyExists: false, selected: true },
  { id: 'imp-2', source: 'tmdb', title: 'Chainsaw Man Part 2 E2', url: '/thumbnails/csm-p2-e2-import.jpg', thumbnailUrl: '/thumbnails/thumbs/csm-p2-e2-import.jpg', resolution: '4k', format: 'jpg', width: 1920, height: 1080, sizeBytes: 780_000, language: 'en', alreadyExists: true, selected: false },
  { id: 'imp-3', source: 'anilist', title: 'Frieren S2 E2', url: '/thumbnails/frieren-s2-e2-import.png', thumbnailUrl: '/thumbnails/thumbs/frieren-s2-e2-import.png', resolution: '1080p', format: 'png', width: 1920, height: 1080, sizeBytes: 1_100_000, language: 'ja', alreadyExists: false, selected: true },
  { id: 'imp-4', source: 'fanart', title: 'Spy x Family Movie 2', url: '/thumbnails/spy-family-m2-import.jpg', thumbnailUrl: '/thumbnails/thumbs/spy-family-m2-import.jpg', resolution: '4k', format: 'webp', width: 1920, height: 1080, sizeBytes: 720_000, language: 'en', alreadyExists: false, selected: true },
  { id: 'imp-5', source: 'tmdb', title: 'Solo Leveling S2 E3', url: '/thumbnails/solo-leveling-s2-e3-import.jpg', thumbnailUrl: '/thumbnails/thumbs/solo-leveling-s2-e3-import.jpg', resolution: '4k', format: 'jpg', width: 1920, height: 1080, sizeBytes: 880_000, language: 'fr', alreadyExists: true, selected: false },
]
...
export default function ThumbnailsPage() {
  ...
  const [inspecting, setInspecting] = React.useState<Thumbnail | null>(null)
  const [importOpen, setImportOpen] = React.useState(false)
  const [settingsOpen, setSettingsOpen] = React.useState(false)
  const [settings, setSettings] = React.useState<ThumbnailSettings>(MOCK_THUMBNAIL_SETTINGS)

  const stats = React.useMemo(() => {
    const total = MOCK_THUMBNAILS.length
    const storage = MOCK_THUMBNAILS.reduce((s, t) => s + t.sizeBytes, 0)
    const unused = MOCK_THUMBNAILS.filter((t) => t.status === 'unused').length
    const missing = MOCK_THUMBNAILS.filter((t) => t.status === 'missing').length
    return { total, storage, unused, missing }
  }, [])

  const filtered = React.useMemo(() => {
    let items = MOCK_THUMBNAILS.filter((item) => {
  ...
  return (
    <main className="flex flex-col gap-6">
      <PageHeader
        title="Thumbnails"
        description="Manage and organize all thumbnail assets across the platform."
      >
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          <span>{stats.total} thumbnails</span>
          <span>·</span>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard label="Total Thumbnails" value={stats.total} icon={<ImageIcon className="size-4 text-muted-foreground" />} />
        ...
        <Select value={mediaTypeFilter} onValueChange={setMediaTypeFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {THUMBNAIL_MEDIA_TYPES.map((t) => (
                <SelectItem key={t} value={t}>{THUMBNAIL_MEDIA_TYPE_LABEL[t]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
        ...
        <Select value={resolutionFilter} onValueChange={setResolutionFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {THUMBNAIL_RESOLUTIONS.map((r) => (
                <SelectItem key={r} value={r}>{THUMBNAIL_RESOLUTION_LABEL[r]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
        ...
        <Select value={sourceFilter} onValueChange={setSourceFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {THUMBNAIL_SOURCES.map((s) => (
                <SelectItem key={s} value={s}>{THUMBNAIL_SOURCE_LABEL[s]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {THUMBNAIL_LANGUAGES.map((l) => (
                <SelectItem key={l} value={l}>{THUMBNAIL_LANGUAGE_LABEL[l]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent>
            <SelectGroup>
              ...
              {THUMBNAIL_STATUSES.map((s) => (
                <SelectItem key={s} value={s}>{THUMBNAIL_STATUS_LABEL[s]}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
      ...
      {filtered.length === 0 ? (
        <Empty className="border">
          <EmptyHeader>
            ...
            <EmptyTitle>No thumbnails found</EmptyTitle>
            <EmptyDescription>No thumbnails match the current filters.</EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : view === 'grid' ? (
        <ThumbnailsGrid items={filtered} onInspect={setInspecting} />
      ) : view === 'table' ? (
        <ThumbnailsTable items={filtered} onInspect={setInspecting} />
      ) : (
        <ThumbnailsGallery items={filtered} onInspect={setInspecting} />
      )}

      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>Showing {filtered.length} of {MOCK_THUMBNAILS.length} thumbnails</span>
      ...
      <ThumbnailDetailSheet item={inspecting} onClose={() => setInspecting(null)} />
...
function ThumbnailPlaceholder({ title, className = '' }: { title: string; className?: string }) {
  return (
...
function ThumbnailsGrid({ items, onInspect }: { items: Thumbnail[]; onInspect: (item: Thumbnail) => void }) {
  return (
      ...
      {items.map((item) => (
        ...
        >
          <div className="relative aspect-video overflow-hidden rounded-md bg-muted">
            {item.status === 'missing' ? (
              <ThumbnailPlaceholder title="Missing" className="size-full" />
            ) : (
              <ThumbnailPlaceholder title={item.title} className="size-full" />
            )}
            <div className="absolute left-2 top-2 flex gap-1">
              <Badge variant={item.status === 'active' ? 'default' : item.status === 'missing' ? 'destructive' : 'secondary'} className="text-[10px]">
                {THUMBNAIL_STATUS_LABEL[item.status]}
              </Badge>
              {item.isDuplicate && (
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>{THUMBNAIL_MEDIA_TYPE_LABEL[item.mediaType]}</span>
              <span>·</span>
              <span>{item.format.toUpperCase()}</span>
              <span>·</span>
              <span>{THUMBNAIL_SOURCE_LABEL[item.source]}</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
...
function ThumbnailsTable({ items, onInspect }: { items: Thumbnail[]; onInspect: (item: Thumbnail) => void }) {
  return (
            ...
            <TableRow key={item.id}>
              <TableCell>
                <button type="button" className="size-10 shrink-0 overflow-hidden rounded bg-muted" onClick={() => onInspect(item)}>
                  <ThumbnailPlaceholder title="" className="size-full" />
                </button>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="hidden md:table-cell">
                <Badge variant="outline" className="text-xs">{THUMBNAIL_MEDIA_TYPE_LABEL[item.mediaType]}</Badge>
              </TableCell>
              <TableCell className="hidden md:table-cell">
                <StatusBadge tone={THUMBNAIL_RESOLUTION_TONE[item.resolution]}>{THUMBNAIL_RESOLUTION_LABEL[item.resolution]}</StatusBadge>
              </TableCell>
              ...
              <TableCell className="hidden xl:table-cell">
                <span className="text-xs">{THUMBNAIL_SOURCE_LABEL[item.source]}</span>
              </TableCell>
              <TableCell className="hidden xl:table-cell">
                <span className="font-mono text-xs">{item.usageCount}</span>
              </TableCell>
              <TableCell className="hidden xl:table-cell">
                <StatusBadge tone={THUMBNAIL_STATUS_TONE[item.status]}>{THUMBNAIL_STATUS_LABEL[item.status]}</StatusBadge>
              </TableCell>
...
function ThumbnailsGallery({ items, onInspect }: { items: Thumbnail[]; onInspect: (item: Thumbnail) => void }) {
        ...
        <Card className="p-4">
          <div className="flex items-center gap-4">
            <div className="aspect-video w-48 shrink-0 overflow-hidden rounded-lg bg-muted">
              <ThumbnailPlaceholder title="Preview" className="size-full" />
            </div>
            <div className="flex-1">
              <h3 className="font-medium">{MOCK_THUMBNAILS.find((t) => t.id === selected)?.title}</h3>
              <p className="text-sm text-muted-foreground">
                {MOCK_THUMBNAILS.find((t) => t.id === selected)?.width}×{MOCK_THUMBNAILS.find((t) => t.id === selected)?.height} · {formatBytes(MOCK_THUMBNAILS.find((t) => t.id === selected)?.sizeBytes || 0)}
              </p>
              <div className="mt-2 flex gap-2">
                <Button size="sm" onClick={() => onInspect(MOCK_THUMBNAILS.find((t) => t.id === selected)!)}>
        ...
        {items.map((item) => (
          ...
          >
            <div className="aspect-video bg-muted">
              {item.status === 'missing' ? (
                <ThumbnailPlaceholder title="Missing" className="size-full rounded-t-lg" />
              ) : (
                <ThumbnailPlaceholder title={item.title} className="size-full rounded-t-lg" />
              )}
            </div>
            <div className="p-2">
...
function ThumbnailDetailSheet({ item, onClose }: { item: Thumbnail | null; onClose: () => void }) {
  return (
            ...
            <Tabs defaultValue="overview" className="flex-1 overflow-y-auto px-4 py-4">
              ...
              <TabsContent value="overview" className="pt-4">
                <FieldGroup>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <MiniStat label="Language" value={THUMBNAIL_LANGUAGE_LABEL[item.language]} />
                ...
                <FieldGroup>
                  ...
                  {item.usedBy.length === 0 ? (
                    <div className="rounded-lg border px-3 py-6 text-center text-sm text-muted-foreground">
                      This thumbnail is not used by any resource
                    </div>
                  ) : (
                        ...
                        <div key={v.id} className="flex items-center justify-between rounded-lg border px-3 py-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-muted-foreground">{THUMBNAIL_LANGUAGE_LABEL[v.language]}</span>
                            </div>
                          </div>
                          <div className="flex items-center gap-4">
              ...
              <TabsContent value="metadata" className="pt-4">
                <FieldGroup>
                  <div className="grid grid-cols-2 gap-4">
                    <MiniStat label="Resolution" value={THUMBNAIL_RESOLUTION_LABEL[item.resolution]} />
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Import Thumbnails</DialogTitle>
          <DialogDescription>Select a source and search for thumbnails to import.</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          <Field>
            ...
            {previews.map((item) => (
              ...
              >
                ...
                {item.alreadyExists ? (
                  ...
                  <Badge variant="secondary" className="text-[10px]">{THUMBNAIL_LANGUAGE_LABEL[item.language]}</Badge>
                )}
              </div>
            ))}
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            onClick={() => {
              toast.success(`${selectedCount} thumbnail(s) imported`)
              onOpenChange(false)
            }}
          >
            <Upload data-icon="inline-start" />
...
function SettingsDialog({ open, onOpenChange, settings, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; settings: ThumbnailSettings; onSave: (settings: ThumbnailSettings) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Thumbnail Settings</DialogTitle>
          <DialogDescription>Configure how thumbnails are managed and processed.</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          <Field>
